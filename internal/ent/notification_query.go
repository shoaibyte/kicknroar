...
func (_q *NotificationQuery) QueryMatch() *MatchQuery {
...
func (_q *NotificationQuery) WithMatch(opts ...func(*MatchQuery)) *NotificationQuery {
	query := (&MatchClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := _q.withMatch; query != nil {
		if err := _q.loadMatch(ctx, query, nodes, nil,
			func(n *Notification, e *Match) { n.Edges.Match = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NotificationQuery) loadMatch(ctx context.Context, query *MatchQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Match)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Notification)
	for i := range nodes {
		if nodes[i].MatchID == nil {
			continue
		}
		fk := *nodes[i].MatchID
		if _, ok := nodeids[fk]; !ok {
...
func (_q *NotificationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withMatch != nil {
			_spec.Node.AddColumnOnce(notification.FieldMatchID)
		}
	}
